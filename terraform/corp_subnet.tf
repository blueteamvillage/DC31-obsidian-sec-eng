...
resource "aws_security_group" "corp_hmail_server" {
  vpc_id      = module.vpc.vpc_id
  description = "Corporate hMail server security group"

  tags = {
    Name    = "${var.PROJECT_PREFIX}_corp_hmail_SERVER_SG"
    Project = var.PROJECT_PREFIX
  }
}
...
resource "aws_security_group_rule" "corp_hmail_allow_teleport" {
  ...
  security_group_id = aws_security_group.corp_hmail_server.id
}
...
resource "aws_security_group_rule" "corp_hmail_allow_prometheus" {
  type        = "ingress"
  description = "Allow Prometheus to access node exporter"
  from_port   = 9100
  to_port     = 9100
...
#tfsec:ignore:aws-ec2-no-public-egress-sgr
resource "aws_security_group_rule" "corp_hmail_allow_egress" {
  type              = "egress"
  description       = "Allow outbound"
  from_port         = 0
  to_port           = 0
...
resource "aws_instance" "corp_hmail_server" {
  ami           = var.windows-ami
  instance_type = "t3.medium"
  subnet_id     = aws_subnet.corp.id
  vpc_security_group_ids = [
    aws_security_group.corp_hmail_server.id,
    aws_security_group.node_exporter_clients.id,
  ]
  key_name   = "${var.PROJECT_PREFIX}-ssh-key"
  private_ip = var.corp_subnet_map["hmail"]
  user_data  = data.template_file.password_change.rendered
  ...
  tags = {
    Name    = "${var.PROJECT_PREFIX}_corp_hmail_SERVER"
    Project = var.PROJECT_PREFIX
  }
}
